...
func (c *Consumer) Start(ctx context.Context) {
	c.log.Info("inventory consumer started", zap.String("queue_url", c.queueURL))

	for {
		select {
		case <-ctx.Done():
			c.log.Info("inventory consumer stopped")
			return
		default:
		}
...
func (c *Consumer) handle(_ context.Context, msg sqstypes.Message) error {
	...
	c.log.Info("processing order",
		zap.Int64("order_id", event.OrderID),
		zap.Int64("customer_id", event.CustomerID),
		zap.Float64("total_amount", event.TotalAmount),
	)

	const stockDeducted = 1
	c.log.Info("inventory updated",
		zap.Int64("order_id", event.OrderID),
		zap.Int("stock_deducted", stockDeducted),
	)
